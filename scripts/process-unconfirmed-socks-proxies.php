<?php
  ...
  exec("cd /var/www/ghostproxies.com/database/unconfirmed-proxies/socks/ && ls -r -t | head -1", $response);
  $proxies = array();
  $proxies_count = 0;
  ...
  if (empty($response) == false) {
    ...
    if (file_put_contents("/var/www/ghostproxies.com/database/unconfirmed-proxies/socks/" . $proxy, "1", FILE_APPEND) != false) {
      $proxy_connection_attempts_count = file_get_contents("/var/www/ghostproxies.com/database/unconfirmed-proxies/socks/" . $proxy);
      ...
      if ($proxy_connection_attempts_count !== false) {
        ...
        if ($proxy_connection_attempts_count < 4) {
          ...
          $process_repetitions_count = 2;

          while ($process_repetitions_count != 0) {
            $copied_proxies_count = $proxies_count;
            usleep(1000);
            $response = array();
            exec("sudo curl -s --insecure --max-time 3 --socks4 " . $proxy . " \"http://parsonsbots.com/?token=" . $token . "\" 2>&1", $response);
            ...
            if (empty($response[0]) == false) {
              ...
              if ($response_proxy_ip_address == $proxy_ip_address) {
                ...
                validate_public_ip_address($response[0]) == true &&
                in_array($response[0] . strstr($proxy, ":"), $proxies) == false &&
                $response_hash == trim(substr(strstr($response[0], "-"), 1))
              ) {
              ...
              usleep(1000);
              $response = array();
              exec("sudo curl -s --insecure --max-time 3 --socks5 " . $proxy . " \"http://parsonsbots.com/?token=" . $token . "\" 2>&1", $response);
        ...
        } elseif (file_exists("/var/www/ghostproxies.com/database/unconfirmed-proxies/socks/" . $proxy) == true) {
          unlink("/var/www/ghostproxies.com/database/unconfirmed-proxies/socks/" . $proxy);
        }
      }
    }
  }
  ...
  if ($proxies_count != 0) {
    while ($proxies_count != 0) {
      ...
      $ip_address_proxy_history = time() . "-" . $proxy_port . "-socks";
